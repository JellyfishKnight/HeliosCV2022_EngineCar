...
DetectorA::DetectorA(Mat &img) : img(img) {}
...
void DetectorA::SetData() const {
    ...
    } else if ((up_down == 0 && left_right == 0) || (up_down == 2 && left_right == 2)) {
        toggle_flag = 0;
    }
}
...
DetectorB::DetectorB(Mat &img) : img(img) {}
...
int DetectorB::Rotate_mine() {
    ...
    for (int i = 0; i < interested_contours.size(); i++) {
        if (rects[i].size.area() < 15000 && rects[i].size.area() > 3000) {
            if (rects[i].size.height / rects[i].size.width >= 0.75 &&
                rects[i].size.height / rects[i].size.width <= 1.25) {
                Point2f points1[4];
                rects[i].points(points1);
                for (int j = 0; j < 4; j++) {
                    line(img, points1[j], points1[(j + 1) % 4], Scalar(0, 0, 255));
                }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (cornerRect[i].size.area() / area >= 1 && cornerRect[i].size.area() / area <= 1.25) {
            check++;
            num = i;
        }
        if (check != 0) {
    ...
    for (auto &i: number) {
        circle(img, centers[i], 2, Scalar(0, 255, 0), 2, LINE_8);
    }
    ...
    if (interestedRect.size() == 2) {
        ...
        for (auto &i: contours_barcode) {
            if (contourArea(i) >= 10000 && contourArea(i) <= 150000) {
                ...
                if(centerRect.size.area() / contourArea(i) <= 2 && centerRect.size.area() / contourArea(i) >= 1) {
                    if ((centerRect.size.height / centerRect.size.width >= 0.4 &&
                         centerRect.size.height / centerRect.size.width <= 0.8) ||
                        (centerRect.size.height / centerRect.size.width >= 1.2 &&
                         centerRect.size.height / centerRect.size.width <= 1.8)) {
                        if(abs(centerRect.center.x - centerofcenter.x) <= 10 && abs(centerRect.center.y - centerofcenter.y) <= 10) {
                            check = 1;
                            Point2f point[4];
                            centerRect.points(point);
                            for (int j = 0; j < 4; j++) {
                                line(img, point[j], point[(j + 1) % 4], Scalar(0, 255, 0), 1, LINE_8);
                            }
                            break;
                        }
                    }
